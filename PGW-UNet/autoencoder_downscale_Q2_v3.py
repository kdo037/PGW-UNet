...
while startDate <= endDate:
    ...
    if os.path.exists(inFile):
        ...
        if init == True:
            ...
            model.summary()
            #model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = 0.001), loss = 'mean_absolute_error', metrics = ['acc'])
            model.load_weights(modDir + 'checkpoint_Q2_v3_2006.weights.h5')
            model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = 0.001), loss = 'mean_absolute_error', metrics = ['acc'])
        
        #model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = 0.001), loss = 'mean_absolute_error',
        #              metrics = ['acc'])
        
        model.fit(train_g, train_c, epochs = 100,batch_size = 10,verbose = 1)
        
        #model.evaluate(test_gray_image,test_color_image)
        
        model.save_weights(modDir + 'checkpoint_Q2_v3_2006.weights.h5')
